...
[2026-09-26] [naming] The CLI ships two bin names, snappy and snappy-prerender, running the same entry point with snappy as the primary command, including the setup wizard as `snappy init`. — user decision: "Can we do snappy init instead?"
[2026-09-26] [naming] The CLI ships a single bin, snappy-prerender, including the setup wizard as `snappy-prerender init`; the short snappy alias was removed. — supersedes entry 2026-09-26 about two bin names.
[2026-09-26] [architecture] This package stays a build-time static prerenderer: no SSR, no runtime rendering, no serve mode. — supersedes entry 2026-09-23 about deferring an SSR renderer to phase two.
[2026-09-26] [naming] The CSP dial is `csp: 'off' | 'strict'` with 'off' as the default; the earlier 'relaxed' name was dropped before release, since it meant the same thing as 'off'. — user decision: "can we add an 'off' setting to the csp dial".
